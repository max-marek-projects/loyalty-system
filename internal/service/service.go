...
func (service *endpointService) AddOrder(ctx context.Context, userID int64, orderNumber string) (bool, error) {
	...
	if !numberValid {
		return false, ErrNumberNotValid
	}
	...
	if err != nil {
		...
		if errors.Is(err, repository.ErrStorageConflict) {
			return false, ErrOrdersConflict
		}
...
func (service *endpointService) worker(
	...
	if err != nil {
		var errorWithRetry *ErrOrderNotYetProcessed
...
func (service *endpointService) parseAccrualSystem(
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			...
			return nil, &ErrOrderNotYetProcessed{fmt.Errorf("timeout received: %w", err), retryAfter}
		}
	...
	case http.StatusOK: // 200
		...
		if result.Status == models.ExternalStatusREGISTERED || result.Status == models.ExternalStatusPROCESSING {
			...
			return nil, &ErrOrderNotYetProcessed{fmt.Errorf("status shows not yet processed"), retryAfter}
		}
		return &result, nil
	case http.StatusNoContent: // 204
		...
		return nil, &ErrOrderNotYetProcessed{fmt.Errorf("status code shows not yet registered"), retryAfter}
	...
	case http.StatusTooManyRequests: // 429
		...
		return nil, &ErrOrderNotYetProcessed{fmt.Errorf("status code shows too many requests"), retryAfter}
